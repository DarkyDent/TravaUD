...
class Keyboard():
    ...
    def comfort_digrams(self):
        ...
        prevf = self.r2
        contf = self.r2
        f = open("digrmas.txt", 'r', encoding='utf-8')
        text = f.readlines()
        for i in text:
            for j in i:
                for k in fins[:5]:
                ...
                for j in i:
                    for k in fins[:5]:
                        if j in k:
                            ...
                            prevf = contf
                            contf = k
                            if fins.index(grandprevf) <= fins.index(prevf) <= fins.index(contf):
                                tri_stats_l += 1
                                break
                    for k in fins[5:]:
                        if j in k:
                            ...
                            if fins.index(grandprevf) >= fins.index(prevf) >= fins.index(contf):
                                tri_stats_r += 1
                                break
        print(di_stats_l, di_stats_r, tri_stats_l, tri_stats_r)
        return di_stats_l, di_stats_r
    ...
    def digrams(self, text):
        ...
        for i in digs:
            ...
            if len(i) >= 2:
                ...
                if f1 is not None and f2 is not None:
                    if f1 < f2 and i not in cdigsl:
                        cdigsl.append(i)
                    elif f1 >= f2: digsl.append(i)
                else:
                    for f in fins[5:]:
                        for F in FINS[4:]:
                            if i[0] in f:
        ...
        for i in trigs:
            ...
            if f1 is not None and f2 is not None and f3 is not None:
                if f1 < f2 < f3 and i not in ctrigsl:
                    ctrigsl.append(i)
                elif f1 >= f2 >= f3:
                    trigsl.append(i)
            else:
                for f in fins[5:]:
                    for F in FINS[4:]:
                        ...
                        if i[2] in f:
                            ...
                            f3 = FINS.index(F) + 1
                if f1 is not None and f2 is not None and f3 is not None:
                    if f1 > f2 > f3 and i not in ctrigsr:
                        ctrigsr.append(i)
                    elif f1 <= f2 <= f3:
                        trigsr.append(i)
        return [digsl, digsr, trigsl, trigsl, cdigsl, cdigsr, ctrigsl, ctrigsr]  #   все вообще двубукв соч-я, трёхб с-я, уд-е 2б для левой руки, уд-е 3б д п.р., уд-е 3б д л.р., уд-е 3б д п.р.