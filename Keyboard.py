            ...
            if f1 is not None and f2 is not None and f3 is not None:
                ...
                if f1 is not None and f2 is not None and f3 is not None:
                    if f1 > f2 > f3:
                        ...
                        trigsr.append(i)
        print(self.name, "всего диграмм: ", len(digs))
        print(self.name, "диграмм, набираемых двумя руками: ", len(digs)-len(digsl)-len(digsr)-len(cdigsr)-len(cdigsl))
        print(self.name, "всего триграмм: ", len(trigs))
        print(self.name, "триграмм, набираемых двумя руками: ", len(trigs) - len(trigsl) - len(trigsr)-len(ctrigsr)-len(ctrigsl))
